...
import sukriti.ngo.mis.databinding.ConfigHomeFloorBinding
...
class HomeFloorConfig : DialogFragment(),NavigationHandler {
    private lateinit var binding: ConfigHomeFloorBinding
    private lateinit var userAlertClient: UserAlertClient
    private lateinit var viewModel: DashboardViewModel
    private lateinit var navigationClient : NavigationClient
    private var selectedTabIndex = 1
    companion object {
        private var INSTANCE: HomeFloorConfig? = null

        fun getInstance(): HomeFloorConfig {
            return INSTANCE
                ?: HomeFloorConfig()
        }
    }
    ...
    override fun onCreateView(
        ...
        binding = ConfigHomeFloorBinding.inflate(inflater, container, false)
        init()
        return binding.root
    }
    ...
    private fun init() {
        ...
        navigateTo(NAV_QUICK_CONFIG_FLOOR_CLEAN)
    ...
    override fun navigateTo(navigationAction: Int) {
        when(navigationAction){
            NAV_QUICK_CONFIG_FLOOR_CLEAN ->{
                navigationClient.loadFragment(FloorConfig.getInstance(),
                    R.id.container,"floorConfig",false)
            }
        }
    }
