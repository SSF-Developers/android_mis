...
class UsageDetails : DialogFragment(),NavigationHandler {
    ...
    companion object {
        private var INSTANCE: UsageDetails? = null

        fun getInstance(): UsageDetails {
            return INSTANCE
                ?: UsageDetails()
        }
    }
    ...
    private fun init() {
        ...
        navigateTo(NAV_USAGE_DETAILS_SUMMARY)
        ...
        binding.tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab?) {
                ...
                when(tab?.position){
                    ...
                    1->{
                        navigateTo(NAV_USAGE_DETAILS_COMPARISON)
                    }

                    2->{
                        navigateTo(NAV_USAGE_DETAILS_TIMELINE)
                    }
                }
            }
    ...
    override fun navigateTo(navigationAction: Int) {
        when(navigationAction){
            NAV_USAGE_DETAILS_SUMMARY ->{
                navigationClient.loadFragment(UsageSummary.getInstance(),
                    R.id.container,"usageSummary",false)
            }
            NAV_USAGE_DETAILS_COMPARISON ->{
                navigationClient.loadFragment(UsageComparison.getInstance(),
                    R.id.container,"usageComparison",false)
            }
            NAV_USAGE_DETAILS_TIMELINE->{
                navigationClient.loadFragment(UsageTimeline.getInstance(),
                    R.id.container,"usageTimeline",false)
            }
        }
    }
