...
import sukriti.ngo.mis.databinding.ConfigFloorCleanBinding
...
class FloorConfig : Fragment() {
    private lateinit var binding: ConfigFloorCleanBinding
    ...
    companion object {
        private var INSTANCE: FloorConfig? = null

        fun getInstance(): FloorConfig {
            return INSTANCE
                ?: FloorConfig()
        }
    }
    ...
    override fun onCreateView(
        ...
        binding = ConfigFloorCleanBinding.inflate(inflater, container, false)
        init()
        return binding.root
    }
    ...
    private fun init() {
        ...
        binding.enabledCont.label.text = "Automatic Floor Clean"
        binding.onCont.label.text = "Floor Clean Duration"
        binding.countCont.label.text = "Floor Clean Count"
        binding.description.text = getString(R.string.description_floor_clean_config)
        ...
        binding.enabledCont.options.onItemSelectedListener = object : OnItemSelectedListener {
            override fun onItemSelected(
                ...
                if (i == 0) {
                    ...
                    binding.countCont.icon.setImageResource(R.drawable.ic_timer_greyed)
                    binding.onCont.time.visibility = View.INVISIBLE
                    binding.countCont.time.visibility = View.INVISIBLE

                    binding.countCont.label.setTextColor(
                    ...
                    binding.countCont.icon.setImageResource(R.drawable.ic_timer)
                    binding.onCont.time.visibility = View.VISIBLE
                    binding.countCont.time.visibility = View.VISIBLE
        ...
        binding.submit.setOnClickListener(View.OnClickListener {
            ...
            if(isSubmissionValidated()){
                ...
                for(index  in 0 until scopeSelection.length()){
                    ...
                    payloadJO.put("Autofloorenabled", selection)
                    payloadJO.put("Floorcleancount", binding.countCont.time.editText?.text.toString())
                    payloadJO.put("Floorcleandurationtimer", binding.onCont.time.editText?.text.toString())
                    ...
                    var payloadInfoJO = CommunicationHelper.getGenericConfigInfo("CMS/FLOOR-CLEAN",
