...
class MiniFlushConfig : Fragment() {
    ...
    companion object {
        private var INSTANCE: MiniFlushConfig? = null

        fun getInstance(): MiniFlushConfig {
            return INSTANCE
                ?: MiniFlushConfig()
        }
    }
    ...
    private fun init() {
        ...
        binding.enabledCont.label.text = "Automatic Mini-Flush"
        binding.onCont.label.text = "Activation Delay"
        binding.offCont.label.text = "Flush Duration"

        binding.description.text = getString(R.string.description_flush_config)
        ...
        val scopeAdapter: ArrayAdapter<*> = ArrayAdapter<Any?>(
            requireContext(),
            R.layout.item_config_selection,
            Nomenclature.getCmsConfigOptions() as List<Any?>
        )
        ...
        binding.submit.setOnClickListener(View.OnClickListener {
            ...
            if(isSubmissionValidated()){
                ...
                for(index  in 0 until scopeSelection.length()){
                    ...
                    payloadJO.put("Autominiflushenabled", selection)
                    payloadJO.put("Miniflushdurationtimer", binding.offCont.time.editText?.text.toString())
                    payloadJO.put("Miniflushactivationtimer", binding.onCont.time.editText?.text.toString())
                    ...
                    var payloadInfoJO = CommunicationHelper.getGenericConfigInfo("CMS/MINI-FLUSH",
