...
class UsageComparisonChartClient(ctx: Context, graph:BarChart, data: UsageReportData) {

    var barChart:BarChart = graph
    var chartData = data.usageComparison
    var context = ctx
    ...
    private fun setData() {
        ...
        if (false) {
            ...
            barChart.getXAxis().setAxisMaximum(
                start + barChart.getBarData()
                    .getGroupWidth(groupSpace, barSpace) * chartData.male.size
            )
